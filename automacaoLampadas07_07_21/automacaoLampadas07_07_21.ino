...
const char* setor1 = "QUARTO 1";
const char* setor2 = "QUARTO 2";
const char* setor3 = "QUARTO 3";
const char* setor4 = "QUARTO 4";
    ...
    if(btnSolto[i]){
      ...
      if(tipoSaida[i] == 1){  //  se o tipo de saida for "1" saida dimmer executa ações para esta saida
        if(timeLongo[i] == tempoEspera){ // ainda nao entrou hold, entao liga ou desliga 100%
          if(fadeIn[i] || fadeOut[i]){ //se ja esta acontecendo fade, para o fade
            fadeIn[i]=0;
            fadeOut[i]=0;
            enviarEstado = 1; //envia alterações para todos clientes
          }
...
void changeOut(String data){
  ...
  if(data.indexOf("<1:") != -1){
    ...
    if(estado.toInt() == 101){
      fadeIn[1] = 0;
      fadeOut[1] = 0;
      if(dimSaida[1] == 0)fadeIn[1] = 1; //se lampada esta desligada, liga lampada 100%
      else fadeOut[1] = 1;
    }
    else{
...
void handleMessage(WebsocketsClient &client, WebsocketsMessage message) { //Recebeu mensagem tratar mensagem
  ...
  if(data.indexOf("changeOut") != -1)changeOut(data);

  if(data.indexOf("renomear") != -1){
    preferences.begin("nomeSetor", false);
    int inicio;
    int fim;
    String nome;
    if(data.indexOf("<1:") != -1){
      inicio = data.indexOf("<1:") +3;
      fim = data.indexOf(">", inicio);
      nome = data.substring(inicio, fim);
      preferences.putString("setor1", nome);
    }
    if(data.indexOf("<2:") != -1){
      inicio = data.indexOf("<2:") +3;
      fim = data.indexOf(">", inicio);
      nome = data.substring(inicio, fim);
      preferences.putString("setor2", nome);
    }
    if(data.indexOf("<3:") != -1){
      inicio = data.indexOf("<3:") +3;
      fim = data.indexOf(">", inicio);
      nome = data.substring(inicio, fim);
      preferences.putString("setor3", nome);
    }
    if(data.indexOf("<4:") != -1){
      inicio = data.indexOf("<4:") +3;
      fim = data.indexOf(">", inicio);
      nome = data.substring(inicio, fim);
      preferences.putString("setor4", nome);
    }    
    preferences.end(); 
    //delay(20);
  }

  if(data.indexOf("verNomes") != -1){    
    String dados="nomes";
    preferences.begin("nomeSetor", false);
    dados = dados + "<1:" + preferences.getString("setor1", "1") + ">";
    dados = dados + "<2:" + preferences.getString("setor2", "2") + ">";
    dados = dados + "<3:" + preferences.getString("setor3", "3") + ">";
    dados = dados + "<4:" + preferences.getString("setor4", "4") + ">";    
    preferences.end();
    client.send(dados);
    Serial.print("mensagem enviada");
...
void bancoDados(){
  preferences.begin("nomeSetor", false);
  if(preferences.getBool("flagSalvo", 0) == 0){
    preferences.putBool("flagSalvo", 1);
    preferences.putString("setor1", setor1);
    preferences.putString("setor2", setor2); 
    preferences.putString("setor3", setor3); 
    preferences.putString("setor4", setor4);    
    Serial.println("Banco criado");
  } 
  else Serial.println("Banco ja salvo");
  preferences.end(); 
}
